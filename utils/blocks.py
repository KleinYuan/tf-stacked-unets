...
def conv2d(inputs, kernel_size, stride, depth, subscope='0'):
    with tf.name_scope("conv2d_{}".format(subscope)):
        ...
        _conv = tf.nn.conv2d(_relu, weights, strides=[1, stride, stride, 1], padding='SAME', name="conv")
...
def unet_module(inputs, subscope='1'):
    ...
    with tf.name_scope("encoder_{}".format(subscope)):
        ...
        conv2 = conv2d(conv1, kernel_size=3, stride=3, depth=32, subscope='{}_2'.format(subscope))
        conv3 = conv2d(conv2, kernel_size=7, stride=3, depth=32, subscope='{}_3'.format(subscope))
        conv4 = conv2d(conv3, kernel_size=11, stride=3, depth=16, subscope='{}_4'.format(subscope))
        conv5 = conv2d(conv4, kernel_size=19, stride=3, depth=16, subscope='{}_5'.format(subscope))

    with tf.name_scope("decoder_{}".format(subscope)):
        deconv1 = deconv2d(conv5, subscope='{}_1'.format(subscope))
        conv6 = conv2d(deconv1, kernel_size=11, stride=3, depth=32, subscope='{}_6'.format(subscope))
        concated = concat(conv3, conv6)
        deconv2 = deconv2d(concated, subscope='{}_2'.format(subscope))
        conv7 = conv2d(deconv2, kernel_size=3, stride=3, depth=64, subscope='{}_7'.format(subscope))
